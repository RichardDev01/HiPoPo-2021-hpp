...
int main (int argc, char *argv[]) {
    ...
    int count_total = 0; // total sum from count
    ...
    printf("Process %d finished in time %f seconds. and %d solutions \n", id, totalTime, count);
    ...
    MPI_Finalize();
    if (id == 0) {
    printf("\nA total of %d solutions were found. on id %d with a time of %f\n", count_total, id,totalTime);
    }
   return 0;
}
